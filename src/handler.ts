...
const calendarHandler: Handler = {
    ScheduleMeeting: {
        START() {
            this.$session.$data.COMPONENT_ScheduleMeeting = {};
        ...
        async DatesIntent() {
             ...
             */
            if (this.$session.$data.COMPONENT_ScheduleMeeting.dates) {
                const dates = this.$session.$data.COMPONENT_ScheduleMeeting.dates;
                ...
                dates.forEach((date: CalendarDate) => {
                    inputs.forEach((input) => {
                        ...
                        if (date.date === dateFromInput) {
                            this.$session.$data.COMPONENT_ScheduleMeeting.currentDateIndex = dates.indexOf(date);
            ...
            const pathToCredentials = path.join(process.cwd(), this.$components.ScheduleMeeting.config.credentialsPath);
            const credentials = require(pathToCredentials);
            const auth = await Authorization.authorize(credentials);
            const calendar = google.calendar({ version: 'v3', auth });
            const timezone = await getTimezone(calendar, this.$components.ScheduleMeeting.config.calendarId);
            ...
            for (const input of inputs) {
                ...
                const events = await getEvents(calendar, {
                    calendarId: this.$components.ScheduleMeeting.config.calendarId,
                ...
                const meetingIntervals: MeetingInterval[] = SlotHelper.computeMeetingIntervals(events, this.$components.ScheduleMeeting.config.intervalTitles);
                ...
                const freeSlots = SlotHelper.computeFreeSlots(meetingIntervals, this.$components.ScheduleMeeting.config.eventLengthInMin);
            ...
            this.$session.$data.COMPONENT_ScheduleMeeting.dates = dates;
            ...
            const date = dates[this.$session.$data.COMPONENT_ScheduleMeeting.currentDateIndex];
        ...
        async SlotIntent() {
            ...
            const date: CalendarDate = this.$session.$data.COMPONENT_ScheduleMeeting.dates[this.$session.$data.COMPONENT_ScheduleMeeting.currentDateIndex];
            ...
            if (slot) {
                this.$session.$data.COMPONENT_ScheduleMeeting.slot = slot;
        ...
        async EventCreationIntent() {
            ...
            const slot: ISlot = this.$session.$data.COMPONENT_ScheduleMeeting.slot;
            const email = this.$components.ScheduleMeeting.data!.email;
            // general: color, reminder, visibility
            const generalEventSettings = this.$components.ScheduleMeeting.config.eventOptions;
